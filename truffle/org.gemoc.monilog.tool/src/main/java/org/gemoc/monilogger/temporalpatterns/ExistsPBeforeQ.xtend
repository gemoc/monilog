...
class ExistsPBeforeQ extends AbstractExistenceProperty {
	...
	private def String rec(int i) {
		'''«IF i == 0»
			nP*? ((EoE | Q) | P«IF exists.bound.eClass.classifierID == MoniLogPackage.LOWER_BOUND»1«ENDIF»)
			«ELSEIF i == 1»
			nP*? ((EoE | Q) | P1«IF exists.bound.eClass.classifierID != MoniLogPackage.LOWER_BOUND» «rec(i - 1)»«ENDIF»)
			«ELSE»
			nP*? ((EoE | Q) | P«IF exists.bound.eClass.classifierID == MoniLogPackage.UPPER_BOUND»1«ENDIF» «rec(i - 1)»)
			«ENDIF»'''
	}
